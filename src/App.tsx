...
const DEFAULT_COURSES: Course[] = [
  { id: '1', name: 'Web Design & Development', category: 'Technical', baseFee: 35000, minFee: 25000, milOfficerBaseFee: 30000, milOfficerMinFee: 22000, milJcoBaseFee: 25000, milJcoMinFee: 18000, instructorName: 'Muhammad Asad', classRoom: 'Computer Lab 1 (Main)' },
  ...
  const notifications = useMemo(() => {
    ...
    records.forEach(r => {
      ...
      if (r.paymentPlan === 'Installment') {
        const calFee = r.totalFee !== undefined ? r.totalFee : calculateTotalFee(r.course || 'Web Design & Development', r.laptop, r.civilStatus, r.discount, r.milCategory).total;
                ...
                <div className="space-y-6 animate-fade-in-quick">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {adminActiveTab === 'analytics' && 'General financial collections ledger totals, conversion tracking analysis, and dynamic student registration metrics.'}
                      {adminActiveTab === 'courses' && 'Establish custom tuition pricing structures, civilian/military floor limits, and assign classroom resources/instructors.'}
                      {adminActiveTab === 'batches' && 'Configure active academic batches, start/end dates, and morning, noon, or evening course configurations.'}
                      {adminActiveTab === 'importer' && 'Mass ingest student registries from Excel / CSV files with automated security floor limit check overrides.'}
                      {adminActiveTab === 'sheets' && 'Configure external Google AppScript webhooks to synchronize admissions databases automatically.'}
                      {adminActiveTab === 'security' && 'Modify system administrator access credentials, passwords, and security settings.'}
                    </p>
                  </div>

                  {/* Modern Tab Bar Selector for Separating Sections */}
                  <div className="flex flex-wrap gap-1.5 border-b border-gray-100 pb-2.5">
                    <button
                      type="button"
                      onClick={() => setAdminActiveTab('analytics')}
                      className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-black transition-all duration-155 cursor-pointer select-none border-0 ${
                        adminActiveTab === 'analytics'
                          ? 'bg-[#004173] text-white shadow-sm font-black'
                          : 'bg-slate-50 text-gray-600 hover:bg-slate-100/80 font-bold'
                      }`}
                    >
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-xs space-y-5 animate-fade-in-quick">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="pt-2 flex flex-wrap gap-2">
                          ...
                          >
                            <Database className="w-3.5 h-3.5" />
                            <span>Run Full Backup Sync Now</span>
                          </button>
                        </div>
